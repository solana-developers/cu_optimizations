...
declare_id!("5N5E4imUxwwdbYZern6xzKXejuHy3jNprfrvVmzT3YdF");
...
pub struct InitializeCounterZeroCopy<'info> {
  #[account(mut)]
  pub payer: Signer<'info>,
  #[account(init, space = 8 + CounterZeroCopy::INIT_SPACE, payer = payer)]
...
pub struct InitPdaWithSeeds<'info> {
  #[account(init, seeds = [b"counter"], bump, payer = signer, space = 8 + CounterData::INIT_SPACE)]
...
pub struct PdaAccounts<'info> {
  ...
  #[account(seeds = [b"counter"], bump = counter_checked.bump)]
  pub counter_checked: Account<'info, CounterData>,
}
...
#[repr(C)]
#[derive(AnchorSerialize, AnchorDeserialize, Copy, Clone, PartialEq, Eq, Zeroable, Pod, InitSpace)]
